...
Vertex AIモードでは `GEMINI_API_KEY` を**渡さない**ことで自動的に認証モードが切り替わります。`GCS_BUCKET_NAME` は20MB超の動画を扱う場合に必須です。サービスアカウントには `roles/aiplatform.user` + `roles/storage.objectAdmin` の付与が必要です。詳細は [環境変数](#環境変数) セクションを参照してください。
...
| `GCS_BUCKET_NAME` | ※3 | 大容量動画の一時アップロード先GCSバケット名（バケット名のみ、`gs://` プレフィックス不要） |

※1 Gemini Developer API 認証の場合に必須
※2 Vertex AI 認証の場合に必須（`GEMINI_API_KEY` が未設定時に自動的にVertex AIモードに切り替わる）
※3 Vertex AIモードで20MB超の動画を扱う場合に必須。バケットは `GEMINI_PROJECT_LOCATION` と同一リージョンを推奨。サービスアカウントには `roles/storage.objectAdmin` （または最低限 `objectCreator` + `objectViewer` + `objectUser`）を付与すること
...
Vertex AI の Gemini API は Gemini Developer API と異なり `files.upload()` を提供しないため、20MB 超の動画は一度 GCS バケットにアップロードしてから `gs://` URI でモデルに渡す必要があります。本MCPは以下を行います:

1. `GCS_BUCKET_NAME` で指定したバケットに `gemini-video-analyze-mcp/<uuid>_<filename>` としてアップロード
2. `Part.from_uri` で Gemini に参照させて解析

アップロードしたBlobは**解析後も残したまま**にします（再利用のため）。後述のクリーンアップ方法を必ず確認してください。
...
20MB超の動画がアップロードされるのはGoogle側のFile APIです。
...
20MB超の動画は `GCS_BUCKET_NAME` で指定したGCSバケットにアップロードされます。
